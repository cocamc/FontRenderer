...
public class FontRenderer {
    ...
    public void drawString(MatrixStack matrices, String s, float x, float y, float r, float g, float b, float a) {
        ...
        matrices.scale(0.125F, 0.125F, 1f); // Adjusted the scaling factor
    ...
    private double drawChar(BufferBuilder bufferBuilder, Matrix4f matrix, char c, float r, float g, float b, float a) {
        ...
        bufferBuilder.vertex(matrix,  0, height * 2,  0).texture(0 + inOffsetX,  1 - inOffsetY).color(r, g, b, a).next();
        bufferBuilder.vertex(matrix, width * 2, height * 2,  0).texture(1 - inOffsetX,  1 - inOffsetY).color(r, g, b, a).next();
        bufferBuilder.vertex(matrix, width * 2,  0,  0).texture(1 - inOffsetX,  0 + inOffsetY).color(r, g, b, a).next();
